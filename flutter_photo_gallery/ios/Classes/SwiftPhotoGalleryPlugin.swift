  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    if(call.method == "listAlbums") {
      ...
      getFile(
        mediumId: mediumId,
        mimeType: mimeType,
        completion: { (filepath: String?, orientation: Int?, error: Error?) -> Void in
            result(["path" :filepath?.replacingOccurrences(of: "file://", with: "") ?? "", "orientation" : orientation ?? 0])
      })
    }
...
//      hideIfEmpty: hideIfEmpty ?? true
//    )
      processPHAssetCollections(fetchResult: PHAssetCollection.fetchAssetCollections(with: .album, subtype: .albumRegular, options: nil), hideIfEmpty: true)
      
      // 同步的照片库
      processPHAssetCollections(fetchResult: PHAssetCollection.fetchAssetCollections(with: .album, subtype: .albumSyncedAlbum, options: nil), hideIfEmpty: true)
    ...
    if (assets.count > 0) {
      ...
      manager.requestImage(
        ...
        resultHandler: {(uiImage: UIImage?, info) in
          ...
          let bytes = image.jpegData(compressionQuality: 0.1)
        ...
        manager.requestImage(
          ...
          resultHandler: {(uiImage: UIImage?, info) in
            guard let image = uiImage else {
              ...
              let bytes = image.jpegData(compressionQuality: 0.5)
            completion(bytes, nil)
        })
        return
      }
  ...
  private func getAlbumThumbnail(
    ...
    let manager = PHImageManager.default()
    let fetchOptions = PHFetchOptions()
    if (mediumType != nil && mediumType != "all") {
    ...
    let assets = albumId == "__ALL__" ?
      ...
      PHAsset.fetchAssets(in: self.assetCollections.first(where: { (collection) -> Bool in
        collection.localIdentifier == albumId
      }) ?? PHAssetCollection.init(), options: fetchOptions)
  ...
  private func getFile(mediumId: String, mimeType: String?, completion: @escaping (String?, Int?, Error?) -> Void) {
    let manager = PHImageManager.default()
          ...
          resultHandler: { (data: Data?, uti: String?, orientation, info) in
            DispatchQueue.main.async(execute: {
              guard let imageData = data else {
                completion(nil, 0, NSError(domain: "photo_gallery", code: 404, userInfo: nil))
                return
              }
              ...
              if mimeType != nil {
                ...
                if type != mimeType {
                  ...
                  completion(path, 0, NSError(domain: "photo_gallery", code: 404, userInfo: nil))
                  return
                }
              }
              ...
              completion(filepath.absoluteString, 0, nil)
            })
          }
        )
        ...
        manager.requestAVAsset(forVideo: asset, options: options, resultHandler: { (avAsset, avAudioMix, info) in
          DispatchQueue.main.async(execute: {
            do {
              ...
              var degress = 0;
              if let track = avAsset?.tracks.first {
              ...
              completion(filepath.absoluteString, degress, nil)
            } catch {
              completion(nil, 0, NSError(domain: "photo_gallery", code: 500, userInfo: nil))
            }
          })
          }
        )
...
//    let filename = self.extractFilenameFromAsset(asset: asset)
      let mimeType = asset.mediaType == .image ? "image" : "video"
      let filename = asset.value(forKey: "filename") as! String
    return [
      "id": asset.localIdentifier,
      "filename": filename,
      "title": self.extractTitleFromFilename(filename: filename),
      "path": "/var/mobile/Media/\(asset.value(forKey: "directory") as! String)/\(asset.value(forKey: "filename") as! String)",
